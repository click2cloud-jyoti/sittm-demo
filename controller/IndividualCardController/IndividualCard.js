...
function savePlane_details(req, res, next) {
    console.log(" savePlane_details -->>", req.body);
    ...
    try {
        ...
        if (req.body.data.full_name) {
            var full_name = req.body.data.full_name;
        }
        if (req.body.data.email) {
            var email = req.body.data.email;
        }
        if (req.body.data.talent_id) {
            var talent_id = req.body.data.talent_id;
        }
        if (req.body.data.manager_email) {
            var manager_email = req.body.data.manager_email;
        }


        config.connect((err, client, done) => {
            if (err) throw err;
            var qurStr = `select _sittm_devlopementPlan_details('${mode}','${email}', ${talent_id},'${manager_email}')`;
            client.query(qurStr, (err, response) => {
                done()
                if (err) throw err;
                if (response) {
                    dataList = response.rows[0]._sittm_devlopementplan_details;
                    res.send(dataList);
                }
...
function UpdateDevlopement_detail(req, res, next) {
    console.log(" UpdateDevlopement_detail -->", req.body);
    ...
    try {
        ...
        async.eachSeries(array, function (eachelement, next) {
            var idd = eachelement.id;
            var talent_id = eachelement.talent_id;
            var mode = 'update';
            var name = eachelement.name;
            var email = eachelement.email;
            var scheduled_on = eachelement.scheduled_on;
            var qurStr = `select _sittm_update_devlopementPlan('${mode}','${email}',${idd},${talent_id},'${name}','${scheduled_on}')`;
...
function DevelopmentDataUpdate(req, res, next) {
    ...
    try {
        ...
        if (req.body.talent_needs_rotaion)
            talent_needs_rotaion = req.body.talent_needs_rotaion;
    ...
    try {
        ...
        config.connect((err, client, done) => {
            if (err) throw err;
                ...
                var qurStr = `select * from user_tags where email = '${email}' and tag_name = '${tag}'`;
                console.log(qurStr);
                client.query(qurStr, (err, response) => {
                    done();
                    if (err) throw err;
...
const STORAGE_ACCOUNT_NAME = 'sittmstoragev1';
const ACCOUNT_ACCESS_KEY = 'Mf0oO2BYwRMfZ8pNfrUAJhqjGqjD8zVsKyyLpRPZQlUs6CyaXmHaROfXjYROxxB+o/UAcyI4zbw8N1nO7RGBUQ==';
